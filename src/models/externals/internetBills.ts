...
export class InternetBill
    ...
    static async generateBills() {
        ...
        const bills = Array.from({ length: 1000 }, (_, i) => {
            const index = i + 3001; // start from 3001 up to 4000
            const dueDate = new Date(today);
            dueDate.setMonth(dueDate.getMonth() + 1); // add 1 month
    
            return {
                bill_id: uuidv4(),
                bill_code: `BILL${String(index).padStart(4, '0')}`, // from BILL3001 to BILL4000
        ...
        console.log('1000 bills (BILL3001 to BILL4000) have been created with due dates 1 month later.');
    }
